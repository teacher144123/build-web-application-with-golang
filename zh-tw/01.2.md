# 1.2 Go Module, GOPATH 與工作空間

從 Go 1.11 開始，Go 官方加入 Go Module 支援，Go 1.12 成為預設支援；此後告別原始碼必須放在 GOPATH 中以及 GOPATH 對初學者造成的困擾。

## 使用 Go Module

如果網路環境特殊，可能需要使用 Proxy 代理，請參考[這裡](/zh/01.2.md)
...
go mod vendor 	# 將依賴轉移到本地的 vendor 文件
...
對於啟用 Go Module 的 Go，需要對 go.mod 進行設定。 (使用 replace 的方式將遠端套件替換成本地套件)
